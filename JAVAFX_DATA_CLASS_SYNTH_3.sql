...
BEGIN
  ...
  add_ln(lc_out, 'public static GridPane getEmptyGrid(List hiddenCols) {');
  add_ln(lc_out, 'GridPane grid = new GridPane();');
  add_ln(lc_out, 'int k = 0;');
  add_ln(lc_out, 'String fieldType = "";');
  ...
  add_ln(lc_out, 'comboBox = new ComboBox(SQLExecutor.getLov("'||upper(par_table_name)||'", dataValues.getString("column_name")));');
  add_ln(lc_out, 'fieldType = "comboBox";');
  add_ln(lc_out, 'break;');
