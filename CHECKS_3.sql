...
BEGIN
  ...
  INTO b_suit_runs VALUES
    (
      NULL, par_chs_code, systimestamp, par_comment
    )
  RETURNING srn_code
  INTO ln_srn_code;
  ...
  FOR rApps IN cApps
  LOOP
    RUN_APPLICATION_CHECKS ( rApps.apl_code, par_chs_code, ln_srn_code );
    sofar := sofar + 1;
...
IS
  ...
  IS
    ...
    FROM b_checks
    WHERE chk_active = 1
    AND chk_chs_code = par_chs_code;
  rChecks cChecks%rowtype;

BEGIN
  FOR rChecks IN cChecks
  LOOP
    checks.register_check(par_apl_code, rChecks, par_srn_code);
  END LOOP;
...
BEGIN
  ...
  INTO b_app_checks VALUES
    (
      NULL, par_apl_code, par_checks.chk_code, ln_check_result, par_srn_code
    );
...
BEGIN
  ...
  LOOP
    doutln(rCheckLovs.clv_value);
    doutln(upper(lc_column_value));
