...
');
  ...
  */
  ...
  FOR rChildren IN cControllerChildren(upper(par_controller))
  loop
  IF (upper(rChildren.ach_view_type) = 'TABLE') THEN
    FOR rTabCols IN csh.cTabCols(rChildren.ach_table)
    loop
      IF (CSH.IS_BOOLEAN_COLUMN(rChildren.ach_table,rTabCols.column_name) = 1) THEN
         doutln(CHECK_BOX_FACTORY(rChildren.ach_table,rTabCols.column_name));
      end if;
    END loop;
  END IF;
  end loop;
