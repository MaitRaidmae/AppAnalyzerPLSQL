...
BEGIN
  ADD_LN(LC_OUT, 'PROCEDURE UPDATE_ROW_NVALUE(par_code NUMBER, par_column VARCHAR2, par_value VARCHAR2)');
  IF (PAR_TYPE = 'HEADER') THEN
    LC_OUT    := LC_OUT || ';';
    RETURN LC_OUT;
  END IF;
...
BEGIN
  ...
  ADD_LN(LC_OUT, 'result_set OUT SYS_REFCURSOR, par_find_by_field VARCHAR2 DEFAULT NULL,');
  add_ln(lc_out, 'par_find_by_value NUMBER DEFAULT NULL)');
  IF (PAR_TYPE = 'HEADER') THEN
    LC_OUT    := LC_OUT || ';';
    RETURN LC_OUT;
  END IF;
  ...
  add_ln(lc_out, 'if (par_find_by_field is null) then');
  ...
  add_ln(lc_out, 'FROM ( SELECT /*+ FIRST_ROWS(n) */');
  add_ln(lc_out, 'tbl.*, ROWNUM rnum');
  ADD_LN(LC_OUT, 'from ( select * from '||par_table_name||' where ''||par_find_by_field||''=''||par_find_by_value||'') tbl');
  ADD_LN(LC_OUT, 'where ROWNUM <=');
  ADD_LN(LC_OUT, '''||ln_last_row||'' )');
  ADD_LN(LC_OUT, 'WHERE RNUM  >= ''||LN_FIRST_ROW;');
  ADD_LN(LC_OUT, 'end if;');

  ADD_LN(LC_OUT, 'OPEN RESULT_SET FOR lc_sql;');
