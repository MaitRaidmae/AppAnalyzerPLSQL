...
BEGIN
  UPDATE B_CHECKS
  SET CHK_MNEMO  = PAR_CHK_MNEMO, CHK_TYPE = PAR_CHK_TYPE, CHK_COMMENT = PAR_CHK_COMMENT, CHK_ACTIVE = PAR_CHK_ACTIVE, CHK_CHS_CODE = PAR_CHK_CHS_CODE
...
BEGIN
  ...
  IF (par_find_by_field IS NULL) THEN
    lc_sql              :='select chk_code,chk_mnemo,chk_type,chk_comment,chk_active,chk_chs_code
...
FROM ( SELECT /*+ FIRST_ROWS(n) */
tbl.*, ROWNUM rnum
from ( select * from B_CHECKS where '||par_find_by_field||'='||par_find_by_value||') tbl
where ROWNUM <=
'||ln_last_row||' )
WHERE RNUM  >= '||LN_FIRST_ROW;
  END IF;
...
BEGIN
  ...
  INTO B_CHECKS
    (
      chk_mnemo, chk_type, chk_comment, chk_active, chk_chs_code
    )
    VALUES
    (
      par_chk_mnemo, par_chk_type, par_chk_comment, par_chk_active, par_chk_chs_code
    );
END INSERT_ROW;
-----------------------------------------------------------------------------
