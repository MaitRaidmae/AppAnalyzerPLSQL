...
BEGIN
  ...
  LOOP
    IF (UPPER(RCHILDREN.ACH_VIEW_TYPE) = 'GRID') THEN
      DOUTLN(GRID_CSS(rChildren.ach_table));
    ELSIF (UPPER(RCHILDREN.ACH_VIEW_TYPE) = 'TABLE') THEN
      DOUTLN(TABLE_CSS(rChildren.ach_table));
    END IF;
  END LOOP;
